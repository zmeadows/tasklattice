...
from tasklattice.core import DomainIntervalUnresolved, Number, type_str_to_type_python
from tasklattice.placeholder.parse import parse_param_str


@pytest.mark.parametrize("x", [1.5, -1.5, 0.0, 0, (-1), 1])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("[-3.,3.]", DomainIntervalUnresolved(-3.0, 3.0, "[", "]"), float),
        ("(0,3]", DomainIntervalUnresolved(0, 3, "(", "]"), int),
        ("(-3.,3.]", DomainIntervalUnresolved(-3.0, 3.0, "(", "]"), float),
    ],
)
def test_parse_domain_types_smoke(
    domain_str: str, domain_parsed: DomainIntervalUnresolved, domain_type: type
) -> None:
...
def test_parse_bool_smoke() -> None:
    ...
    type_actual = type_str_to_type_python(pu.py_type)
